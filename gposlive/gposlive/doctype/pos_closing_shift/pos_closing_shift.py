...
class POSClosingShift(Document):
    def validate(self):
        ...
        if user:
            frappe.throw(
                _(
                    "POS Closing Shift {0} against {1} between selected period"
                ).format(
...
@frappe.whitelist()
def get_pos_invoices(pos_opening_shift: str) -> list[str]:
...
@frappe.whitelist()
def get_payments_entries(pos_opening_shift: str) -> list[dict]:
...
@frappe.whitelist()
def submit_closing_shift(closing_shift: str) -> dict:
